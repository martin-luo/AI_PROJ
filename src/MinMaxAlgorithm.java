public class MinMaxAlgorithm
{
	...
	//first one will get call second one to expamd all possible move and get best one 
	...
	public void makeBestMove(Board board, int depth,int branch,String myPlayer,String opponentPlayer)
	{
		...
		this.opponenetPlayer=opponenetPlayer;
		...
		outerLoop: while(true)
		{
			...
			//for visited node and need to go down without evaluation until hit the las one .
			
			//if(currentTranverseNode.visited==true && currentTranverseNode.children.size()!=0)
			//jump back to visited node just go down ...if come to here means there must be a children in this node.
...
//				}
	...
	public boolean isPrunningTrue(TreeNode currentTranverseNode)
	{
		...
		{
			//if parent has utility , and if this is min node , if it is not bigger than parent(max) then we can prune the rest of tree
			if(currentTranverseNode.parent.utilityValue!=-1&&currentTranverseNode.utilityValue<currentTranverseNode.parent.utilityValue)
			{
				return true;
			}
...
//	}
	...
	public boolean setupBranch(TreeNode currentNode,int branch)
	{
		...
		int [] tempPossibleMove=currentNode.board.getOnePossibleMove(myPlayer,opponenetPlayer,pickBestBranchMoveMethod);
		...
		for(int i=0;i<branch-1&&tempPossibleMove!=null;i++)
		{
			System.out.println("child movement row = "+tempPossibleMove[0]+" col = "+tempPossibleMove[1]);

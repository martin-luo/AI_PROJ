...
public class CellNode
{
	 ...
	 *            its prev y
	 * @param whoseCircle
	 *            onwer of this circle
	 * @param boardDimension
	 * @param boardBody
	 */

	CellNode(int currentNodeX, int currentNodeY, int prevNodeX, int prevNodeY,
			String whoseCircle, int boardDimension, String[][] boardBody)
	{
	 ...
	 * 
	 * @param whoseCircle
	 *            is used to determine who is the owner of this circle
	 */
	...
	public void fillValidAdjcentCell(String whoseCircle)
	{
		...
		for (int i = 0; i < directionList.length; i++)
		{
			...
			// System.out.println("temp position sisgn ===> "+boardBody[tempY][tempX]);
			...
			{
				// with respect to current node .. all its surrounding which is
				// not exceed board limites will be added start from top left
				...
				// System.out.println("most clock ==> current x = "+currentNodeX+"current y = "+currentNodeY+"tempX = "+tempX+"temp Y = "+tempY+" Sign= "+boardBody[tempY][tempX]);
			}
		}
	...
	/**
	 * clockwise is based on this cell's previous cell ,so this function get
	 * index of prev cell in 8 directions cell array
	 * 
	...
	public int prevIndexInValidAdj()
	{
		...
		{
			if (prevNodeX == validAdjCellX.get(i)
					&& prevNodeY == validAdjCellY.get(i))
			{
				index = i;
				return index;
			}
	 ...
	 * 
	 * @return vadlidXY its first index is valid adjacent cell's x , second
	 *         index is y
	 */
	...
	public int[] mostClockwisedCellIndex()
	{
		...
		{
			if (prevNodeX == -1 && prevNodeY == -1 && validAdjCellX.size() > 0)
			{
	...
	// simply return 8 diretions of connected cell
	public void fillValidAdjcentCell()
	{
		...
		for (int i = 0; i < directionList.length; i++)
		{
			...
			// System.out.println("tempX = "+tempX+"temp Y = "+tempY);
			// System.out.println(""+"current x = "+currentNodeX+"current y = "+currentNodeY+""+(boardBody[tempY][tempX].equals(whoseCircle))+" sign "+boardBody[tempY][tempX]);
			if (!boardBody[tempY][tempX].equals(Board.FREE)
					&& !boardBody[tempY][tempX].equals(Board.BLACKCAPTURED)
					&& !boardBody[tempY][tempX].equals(Board.WHITECAPTURED))
			{
				// with respect to current node .. all its surrounding which is
				// not exceed board limites will be added start from top left
				// in a clock wise direction to tempValidAdjacent
